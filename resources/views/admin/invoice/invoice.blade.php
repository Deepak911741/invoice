...
<main class="page-height bg-light-color">
    <div class="breadcrumb-wrapper d-flex border-bottom">
        ...
        <div class="ms-auto pt-sm-0 d-flex align-items-center">
            <a href="{{ config('constants.INVOICE_URL') . '/create' }}" class="d-flex align-items-center btn btn-theme text-white button-actions-top-bar add-btn  border btn-sm me-2 twt-add-btn" title="{{ trans('messages.add-invoce') }}"><i class="fas fa-plus twt-add-icon"></i> <span class="d-sm-block d-none"> {{ trans("messages.add-invoce") }} </span></a>
    ...
    <section class="inner-wrapper-common-section main-listing-section pt-3">
        <div class="container-fluid">
            <?php
            $searchPlaceholder = trans('messages.search-by-module' , [ 'Module' => implode(", " , [ trans('messages.name'), trans('messages.email-id')] ) ]);
            ?>
                    ...
                    <div class="row align-items-center">
                        ...
                        <div class="col-lg-3 col-sm-6">
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control twt-enter-search custom-input" name="search_date" placeholder="{{ config('constants.DEFAULT_DATE_FORMAT')}}">
                            </div>
                        </div>
            ...
            <div class="filter-result-wrapper">
                ...
                {{ Message::readMessage() }}
                    <div class="table-responsive">
                        <table class="table table-sm table-bordered table-hover w-100" id="invoice-table">
                            <thead class="twt-table-header">
                                <tr>
                                    <th class="sr-col">{{ trans("messages.sr-no") }}</th>
                                    <th>{{ trans("messages.name") }}</th>
                                    <th class="">{{ trans("messages.mobile-no") }}</th>
                                    <th>{{ trans("messages.date") }}</th>
                                    <th>{{ trans("messages.total-due-payment") }}</th>
...
var module_url = '{{ config("constants.INVOICE_URL") }}' + '/';
...
function reintDataTable(class_name = null) {
    ...
    table_columns.push({ data: 'date' });
    table_columns.push({ data: 'due_payment' });
...
$(document).ready(function() {
    ...
    $("[name='search_date']").datetimepicker({
        useCurrent: false,
        viewMode: 'days',
        ignoreReadonly: true,
